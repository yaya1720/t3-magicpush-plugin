...
class MagicPushAutomationPlugin(BasePlugin, AutomationProvider):
    ...
    def test(self, ctx: dict[str, Any]) -> dict[str, Any]:
        ...
        title = "🔔 测试消息"
        content = "这是一条来自 T3FAP 魔法推送插件的测试消息。\n\n如果你收到这条消息，说明配置成功！"
        result = self._send_notification(title, content)
        return {
            "success": result["success"],
    ...
    def _send_notification(self, title: str, content: str) -> dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                return {"success": True, "message": f"推送成功，状态码：{response.status_code}", "status_code": response.status_code}
            else:
                return {"success": False, "message": f"推送失败，状态码：{response.status_code}，响应：{response.text[:200]}", "status_code": response.status_code}
