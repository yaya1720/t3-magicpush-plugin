...
class MagicPushAutomationPlugin(BasePlugin, AutomationProvider):
    ...
    def health(self, ctx: dict[str, Any]) -> dict[str, Any]:
        """健康检查 - 在插件界面点击「健康检查」时触发"""
        configured = self._is_configured()
        return {
            "status": "ok" if configured else "misconfigured",
            "message": "✅ 魔法推送通知插件运行正常。" if configured else "⚠️ 插件尚未配置完整，请填写 Webhook 地址和认证令牌。",
            "details": {
                "configured": configured,
                "subscribed_events": self.subscribed_events(),
                "url": self._runtime_config.get("url") if configured else None,
                "token_masked": self._runtime_config.get("token")[:4] + "****" if configured and self._runtime_config.get("token") else None,
    ...
    def handle(self, event: dict[str, Any]) -> dict[str, Any]:
        ...
        result = self._send_notification(title, content)
        return OperationResult(
            success=result["success"],
            message=f"{self.plugin_name} 已处理事件：{event_type}。{result['message']}",
            data={
                "configured": self._is_configured(),
                "request_payload": {"title": title, "content": content, "type": "text"},
                "response_status": result.get("status_code"),
                "timestamp": datetime.datetime.now().isoformat(),
            },
        ).model_dump(mode="json")

    def test(self, ctx: dict[str, Any]) -> dict[str, Any]:
        """
        发送测试消息 - 在插件界面点击「发送测试消息」时触发
        """
        if not self._is_configured():
            return {
                "success": False,
                "message": "❌ 插件未配置，请先填写 Webhook 地址和认证令牌。",
                "data": {"configured": False},
            }

        title = "🔔 T3FAP 魔法推送测试"
        content = (
            "这是一条来自 T3FAP 魔法推送插件的测试消息。\n\n"
            "✅ 如果你收到这条消息，说明配置完全正确！\n"
            f"🕐 发送时间: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
        ...
        return {
            ...
            "data": {
                ...
                "token_masked": self._runtime_config.get("token")[:4] + "****" if self._runtime_config.get("token") else None,
                "timestamp": datetime.datetime.now().isoformat(),
            },
        }

    def _send_notification(self, title: str, content: str) -> dict[str, Any]:
        """发送通知到魔法推送"""
        url = self._runtime_config.get("url")
        token = self._runtime_config.get("token")
        if not url or not token:
            return {"success": False, "message": "配置不完整：缺少 url 或 token"}
        ...
        try:
            ...
            if response.status_code == 200:
                return {
                    "success": True,
                    "message": f"✅ 推送成功，状态码：{response.status_code}",
                    "status_code": response.status_code,
                }
            else:
                return {
                    "success": False,
                    "message": f"❌ 推送失败，状态码：{response.status_code}，响应：{response.text[:200]}",
                    "status_code": response.status_code,
                }
